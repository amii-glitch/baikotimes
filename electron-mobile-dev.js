...
const MOBILE_WIDTH = Number(process.env.MOBILE_WIDTH || 390);
const MOBILE_HEIGHT = Number(process.env.MOBILE_HEIGHT || 844);
const MOBILE_UA =
...
function openMobileWindow(targetUrl) {
  const sizeArg = `--window-size=${MOBILE_WIDTH},${MOBILE_HEIGHT}`;
  const launchers = [
    ['cmd', ['/c', 'start', '', 'chrome', `--app=${targetUrl}`, sizeArg, `--user-agent=${MOBILE_UA}`]],
    ['cmd', ['/c', 'start', '', 'msedge', `--app=${targetUrl}`, sizeArg, `--user-agent=${MOBILE_UA}`]],
...
(async () => {
  ...
  setTimeout(() => openMobileWindow(url), 1200);
